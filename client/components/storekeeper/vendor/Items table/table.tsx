...
const INITIAL_VISIBLE_COLUMNS = ["vendor_name", "phone_no", "email", "actions"];
...
export const TableWrapper = () => {
  ...
  const renderCell = React.useCallback((user: User, columnKey: React.Key) => {
    ...
    switch (columnKey) {
      case "actions":
        return (
          <Link href={`/storekeeper/vendor/${user._id}`}>
            <ActionIcon />
          </Link>
        );
      default:
  ...
  const getAllVendors = async () => {
    try {
      const response = await axios.get("http://localhost:5000/vendor/all");
