...
import { capitalize } from "../location/Items table/utils";
...
export const IssueItemPage = () => {
  ...
  const getAllLostDamaged = async () => {
    try {
      const response = await axios.get('http://localhost:5000/request/lost-and-damaged');
      if(response){
          ...
          if (product.returned_status === "Damaged") {
            return (
              <div className="text-lg">
                <DeleteModel description={product.damaged_lost_description} name={product.item_id.item_name}/>
              </div>
            );
          }
        default:
