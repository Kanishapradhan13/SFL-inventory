    ...
    return (
      <div className="flex gap-4 h-full items-center cursor-pointer ">
        ...
        <AccordionItem
          classNames={{
            base: clsx(
              activePage.includes("/storekeeper/request") && "bg-warning text-white [&_svg_path]:fill-white border rounded-lg "
            ),
            ...
            title={
            <div className={clsx("flex flex-row gap-2", activePage.includes("/storekeeper/request") && "text-white")}>
                <span >{icon}</span>
                <span className={clsx("text-default-400 text-xs", activePage.includes("/storekeeper/request") && "text-white")}>{title}</span>
