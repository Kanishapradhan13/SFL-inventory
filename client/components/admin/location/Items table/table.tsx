...
import { DeleteIcon } from "../Items table/DeleteIcon.jsx";
...
export const TableWrapper = () => {
  const [columns, setColumns] = React.useState([
    ...
    { name: "LOCATION", uid: "location_name", sortable: true },
    { name: "NO OF SUB LOCATIONS", uid: "no_of_sublocations", sortable: true },
  ...
  const filteredItems = React.useMemo(() => {
    ...
    if (hasSearchFilter) {
      filteredUsers = filteredUsers.filter((user) =>
        user["location_name"].toLowerCase().includes(filterValue.toLowerCase())
      );
    }
  ...
  const renderCell = React.useCallback((user: User, columnKey: React.Key) => {
    ...
    switch (columnKey) {
      case "actions":
        return (
          <div>
            <Link href={`/admin/location/${user._id}`}>
              <ActionIcon />
            </Link>
          </div>
        );
  ...
  const getAllLocations = async () => {
    try {
      const response = await axios.get(
        "http://localhost:5000/location/all"
      );
