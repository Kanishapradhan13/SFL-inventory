...
export const TableWrapperReturn = () => {
  ...
  const getAllProducts = async () => {
    try {
      const response = await axios.get('http://localhost:5000/request/loan-returned')
  ...
  const filteredItems = React.useMemo(() => {
    ...
    if (hasSearchFilter) {
      filteredProducts = filteredProducts.filter((product) =>
        product.item_id.item_name.toLowerCase().includes(filterValue.toLowerCase())
      );
    }
  ...
  const renderCell = React.useCallback(
    (product: Product, columnKey: React.Key) => {
      ...
      switch (columnKey) {
        ...
        case "requested date":
          return product.createdAt &&  new Date(product.createdAt).toISOString().substring(0, 10);
          case "returned date":
            return product.updatedAt &&  new Date(product.updatedAt).toISOString().substring(0, 10);
        default:
          return cellValue;
      }
    },
