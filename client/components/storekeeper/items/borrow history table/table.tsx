...
export const BorrowHistoryTableWrapper = ({itemId}) => {
  ...
  const getAllDetails = async () =>{
    try {
      const response = await axios.get(`http://localhost:5000/request/borrow-history/${itemId}`)
      if(response){
  ...
  const renderCell = React.useCallback(
    (product: Product, columnKey: React.Key) => {
      ...
      switch (columnKey) {
        ...
        case "Issued by":
          return (
            product.issuerDetails.name
          )
        case "status":
          return (
            product.status == "Unreturned" ? "Failed to Return" : product.status
          );
        default:
          return cellValue;
      }
